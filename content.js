...
// this function is used to fetch notes from the server
class TabManager {
    constructor() {
        this.tabs = {
    ...
    createFlashCard() {
        ...
        contentWrapper.style.cssText = `
            ...
            position: relative; /* Make the contentWrapper relative to position buttons inside */
        `;
    
        const loadingMessage = document.createElement("div");
        loadingMessage.innerText = "Take a deep breath ✨✨";
        setTimeout(() => {
            loadingMessage.innerText = "Generating Flashes for you...";
        }, 2000);
        ...
        fetchFlash().then((flash) => {
            ...
            function updateFlashCard() {
                ...
                // Remove existing flashcard content
                const flashContent = contentWrapper.querySelector(".flash-content");
                if (flashContent) {
                    contentWrapper.removeChild(flashContent);
                }
    
                // Create a new flashcard
                const flashCard = document.createElement("div");
                flashCard.classList.add("flash-content");
                ...
                `;
                frontSide.innerHTML = `<div>${currentIndex +1  + " "}${flash.question}</div>`;
                ...
                `;
                backSide.innerHTML = `<div>${flash.answer}</div>`;
                ...
                flashCard.appendChild(innerContainer);
    
                // Add click event for flip animation
                flashCard.addEventListener("click", () => {
                    innerContainer.style.transform = innerContainer.style.transform === "rotateY(180deg)" 
                        ? "rotateY(0deg)" 
                        : "rotateY(180deg)";
                });
    
                // Append flashcard to the content wrapper
                contentWrapper.appendChild(flashCard);
            }
    ...
    injectCustomDiv() {
        ...
        tabButtonsContainer.style.cssText = "display: flex; gap: 8px; margin-bottom: 12px; overflow-x: scroll";
...
async function fetchFlash() {
    ...
    try {
        ...
        return JSON.parse(formattedResponse.substring(7, formattedResponse.length - 4))
    } catch (error) {
        console.error("Error:", error);
        return "Error fetching response.";
    }
